...
DisplayCPU::DisplayCPU(DisplayCPU const & src) {
	*this = src;
	return ;
}

DisplayCPU		&DisplayCPU::operator=(DisplayCPU const & src)
{
	if (this != &src)
		*this = src;
	return (*this);
...
void			DisplayCPU::set_empty(int n) {
	this->_empty = n;
	return ;
}

int				DisplayCPU::get_empty(void) const {
	return (this->_empty);
}
